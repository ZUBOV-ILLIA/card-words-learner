...
export default function AddEditWords({ word, children }: { word?: Word, children?: React.ReactNode }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [foreignWord, setForeignWord] = useState(word?.foreign ? word.foreign : "");
    ...
    <>
      ...
      {isModalOpen && createPortal(
        <Modal>
          ...
          />
          <div
            className="w-11/12 flex flex-col bg-white p-4 rounded-lg z-10"
          >
            <input
              type="text"
              name="foreign-word"
